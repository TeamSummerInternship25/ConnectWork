...
router.get('/organizer', (0, auth_1.requireRole)(['ORGANIZER']), async (req, res, next) => {
    try {
...
router.get('/speaker', (0, auth_1.requireRole)(['SPEAKER']), async (req, res, next) => {
    try {
...
router.get('/audience', (0, auth_1.requireRole)(['AUDIENCE']), async (req, res, next) => {
    try {
        ...
        }).filter(user => user.totalAnswered > 0)
...
router.get('/presentation/:presentationId', (0, auth_1.requireRole)(['ORGANIZER', 'SPEAKER']), async (req, res, next) => {
    try {
        ...
        const audienceStats = presentation.audience.map(audienceMember => {
            const userAnswers = presentation.quizzes.flatMap(quiz => quiz.answers.filter(answer => answer.userId === audienceMember.userId));
...
router.get('/trends/:presentationId', (0, auth_1.requireRole)(['ORGANIZER', 'SPEAKER']), async (req, res, next) => {
    try {
        const { presentationId } = req.params;
        const userId = req.user.id;
        const presentation = await prisma.presentation.findUnique({
        ...
        const trends = Object.values(timeIntervals).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
        res.json({ trends });
    }
    catch (error) {
        next(error);
